...
for j in range(customer_num):
    ...
    d[j] = DP.addVar(lb=demand_nominal[j],vtype=GRB.CONTINUOUS,name=f'd_{j}')
...
for i in range(facility_num):
    expr.addTerms(1, z[i])
master.addConstr(expr >= sum(d_sol))  # 206 + 274 + 220 + 40 * 1.8
#
...
for i in range(facility_num):
    pi[i] = subProblem.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name='pi_%s' % i)
...
#     g[j] = subProblem.addVar(lb=0, ub=1, vtype=GRB.CONTINUOUS, name='g_%s' % j)
    theta[j] = subProblem.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name='theta_%s' % j)
...
for j in range(facility_num):
    ...
    for i in range(customer_num):
        expr.addTerms(1, x[i, j])
    subProblem.addConstr(expr >= d_sol[j])
...
for j in range(customer_num):
    ...
    for i in range(facility_num):
        expr.addTerms(1, x[i, j])
    subProblem.addConstr(expr - d_sol[j] <= big_M - big_M * w[j])
#
#
# logic 3
for j in range(customer_num):
...
#
        ...
        if (subProblem.status == 2):
            UB = subProblem.ObjVal + master.ObjVal - eta.x
